...
function Navbar() {
    ...
    const navItems = [
        { path: '/user/feed', label: 'Feed', icon: Activity },
        { path: '/user/connections', label: 'Connections', icon: Users },
        { path: '/user/requests/recieved', label: 'Requests', icon: Bell },
    ];
    ...
    return (
        ...
        <div className="fixed top-2 md:top-4 left-1/2 -translate-x-1/2 z-50 w-[96%] md:w-[95%] max-w-6xl navbar h-14 md:h-16 min-h-0 py-0 bg-base-100/80 backdrop-blur-xl shadow-lg rounded-full px-3 md:px-8 border border-base-content/10 text-base-content transition-all">
            ...
            <div className="navbar-start">
                <Link to={user ? "/user/feed" : "/"} className="text-lg md:text-2xl font-extrabold tracking-tight flex items-center gap-1.5 md:gap-2 hover:opacity-80 transition-opacity">
            ...
            <div className="navbar-end">
                {!user ? (
                    <div className="flex items-center gap-2">
                        <Link to='/user/login' className="btn btn-ghost min-h-0 h-9 px-3 md:h-10 md:px-4 text-xs md:text-sm rounded-full text-base-content/80 hover:text-base-content hover:bg-base-content/10 font-semibold">
                            Log in
                        </Link>
                        <Link to='/user/signup' className="btn btn-primary min-h-0 h-9 px-4 md:h-10 md:px-6 text-xs md:text-sm rounded-full font-bold shadow-lg shadow-primary/30">
                            Sign up
                        </Link>
                    </div>
                ) : (
                    <div className="flex items-center gap-2 md:gap-4">
                        ...
                        <div className="dropdown dropdown-end">
                            ...
                            <ul tabIndex={0} className="menu menu-sm dropdown-content bg-base-200 border border-base-content/10 rounded-box z-[50] mt-3 md:mt-4 w-52 p-2 shadow-2xl font-medium text-base-content">
                                <li>
                                    <Link to='/user/profile' className="py-2.5 md:py-3 hover:bg-base-300 flex items-center gap-3">
                                        <UserIcon className="w-4 h-4 md:w-5 md:h-5 opacity-70" /> Profile
                                    </Link>
                                </li>
                                <li>
                                    <Link to='/user/account' className="py-2.5 md:py-3 hover:bg-base-300 flex items-center gap-3">
